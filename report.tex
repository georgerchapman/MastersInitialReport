...
\end{equation}
where $\Re$ denotes the real part, $A$ is the wave amplitude at each impact, $\tau$ is the typical decay time and $\bm{r_n}$ and $t_n$ are the position and time of previous bounces, respectively.  As $\gamma_m$ is increased towards $\gamma_m^F$, $\tau$ is increase and the length of the path memory is therefore increased.
